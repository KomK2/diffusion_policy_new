...
class Block(nn.Module):
    ...
    def forward(self, x, T):
        ...
        cls_token = rearrange(cls_token, "b t m -> (b t) m", b=B, t=T).unsqueeze(
            1
        )  # [(b t) 1 m]
        xs = xt
        ...
        for stride, kernel_width in zip(strides, kernel_widths):
            self.layers.append(
                nn.Conv1d(
                    ...
                    out_channels=self.encoding_dim,
...
class Spec_Patcher(nn.Module):
    ...
    def forward(self, waveform):
        ...
        log_spec = rearrange(
            log_spec, "b c m (l np) -> b l np (c m)", np=self.num_patches
        )
        return log_spec
        ...
        if self.learn_time_embedding:
            ...
            for i in range(x.size(0)):
                x[i] = (
                    x[i]
                    + self.pe[
                        0, start[i] + self.offset : start[i] + x.size(1) + self.offset
                    ]
                )
        return self.dropout(x)


class MultiModal_TimeSformer(nn.Module):
    def __init__(
        self,
        *,
        image_size,
        ...
        self.to_patch_embedding_v = nn.Sequential(
            Rearrange(
                "b l c (h p1) (w p2) -> b l (h w) (p1 p2 c)",
                p1=patch_height,
                p2=patch_width,
            ),
        ...
        # num_patches_t = (tactile_height // patch_height) * (
        #     tactile_width // patch_width
        # )
        print(
            "# of audio patches",
            # num_patches_a,
    ...
    def forward(self, inputs, start):
        ...
        if "vf" in self.modalities:
            vf_patch = self.to_patch_embedding_v(
                vf_inp
            )  # [batch, num_frames, num_patches_v, dim]
        ...
        #     ah_patch = self.to_patch_embedding_a(
        #         audio_h
        #     )  # [batch, T // prod(strides), dim]
        #     ah_patch = ah_patch[:, -a_patches_to_keep:, :].view(
        #         batch_size, num_frames, self.num_patches_a, ah_patch.size(-1)
        #     )
        ...
        #     ag_patch = ag_patch[:, -a_patches_to_keep:, :].view(
        #         batch_size, num_frames, self.num_patches_a, ag_patch.size(-1)
        #     )
        ...
        embeds = torch.cat(
            embeds, dim=2
        )  # [batch, num_frames, total_patches, dim + modal_enc_dim]
        ...
        embeds = embeds.view(
            batch_size, num_frames * total_patches, -1
        )  # [batch, num_frames * total_patches, dim]
...
def test_multimodal_timesformer():
    # Hard-coded arguments from YAML configuration and command-line arguments
    resized_height_v = 128
    resized_width_v = 96
    ...
    use_mha = True
    action_dim = 3
    num_classes = 3 ** action_dim
    fps = 10
    channels = 3
    audio_channels = 2
    norm_audio = True

    # Instantiate the MultiModal_TimeSformer with hard-coded values
    model = MultiModal_TimeSformer(
        image_size=(resized_height_v, resized_width_v),
        ...
        drop_rate=0.0,  # This was not defined, setting to default (0.0)
        attn_drop_rate=0.0,  # This was not defined, setting to default (0.0)
        drop_path_rate=drop_path,
        use_1dconv=True,  # Default is True, based on common transformer settings
        learn_time_embedding=True  # From YAML file
    ).cuda()


    vf_inp = torch.zeros((16, 30, 3, 128, 96)).float().cuda()
    # vg_inp = torch.zeros((3, 30, 3, 128, 96)).float().cuda()

    # t_inp = torch.zeros((3, 30, 3, 128, 96)).float().cuda()

    # audio_h = torch.zeros((3, 2, 16000 * 150)).float().cuda()
    # audio_g = torch.zeros((3, 2, 16000 * 150)).float().cuda()
