...
if __name__ == "__main__":
    ...
    model = DiTNoiseNet(ac_dim=6, ac_chunk=16,hidden_dim=1542).to("cuda:0")
    x = torch.randn(16, 16, 6).to("cuda:0")
    t = torch.randn(16).to("cuda:0")
    cond = torch.randn(16, 2, 1542).to("cuda:0")

    enc_cache, out = model(x, t, cond)

    print("output shape:", out.device)
...
class DiffusionTransformerHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        print("hidden_dim",obs_feature_dim) 

        model = DiTNoiseNet(ac_dim=input_dim, ac_chunk=horizon,hidden_dim=obs_feature_dim,nhead=7 )
    ...
    def conditional_sample(self, 
        ...
        for t in scheduler.timesteps:
            ...
            # t = t.to(self.device)
            y= t.to(self.device)
            y= y.unsqueeze(0).repeat(condition_data.shape[0])

            # 2. predict model output
            enc , model_output = model(trajectory, y, cond)
    ...
    def compute_loss(self, batch):
        ...
        enc, pred = self.model(noisy_trajectory, timesteps, cond)
