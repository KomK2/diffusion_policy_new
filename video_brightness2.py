...
def process_video(args):
    original_file_path, new_file_path, filename = args
    print(f"Processing: {original_file_path}")
    if filename == "2.mp4":
        # Increase brightness for videos named '2.mp4'
        subprocess.run([
        ...
        # Copy other videos without modification
        subprocess.run(["cp", original_file_path, new_file_path])
...
for root, dirs, files in os.walk(ORIGINAL_DIR):
    ...
    for file in files:
        if file.endswith(".mp4"):
            ...
            video_files.append((original_file_path, new_file_path, file))
